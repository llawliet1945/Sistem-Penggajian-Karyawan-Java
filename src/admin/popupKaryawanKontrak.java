...
public class popupKaryawanKontrak extends javax.swing.JFrame {
    private DefaultTableModel tabmode;
    private static Connection conn = new connect().configDB();
    public Admin KK = null;
    public Admin THRKK = null;
    public popupKaryawanKontrak() {
        initComponents();
        DataKK();
    }
    ...
    private void tbKaryawanKontrakMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbKaryawanKontrakMouseClicked
        ...
        KK.nik = tbKaryawanKontrak.getValueAt(row, 0).toString();
        KK.nama = tbKaryawanKontrak.getValueAt(row, 1).toString();
        KK.jabatan = tbKaryawanKontrak.getValueAt(row, 5).toString();
        try{
            String SQL = "SELECT * FROM jabatan where Kode_Jabatan='"+KK.jabatan+"'";
            Statement State = conn.createStatement();
            ResultSet Result = State.executeQuery(SQL);
            while (Result.next()) {
                KK.gapok = Result.getInt("Gaji_Pokok");
            }
        }catch(SQLException yusuf){
            JOptionPane.showMessageDialog(this, "data gagal dipanggil"+yusuf);
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new popupKaryawanKontrak().setVisible(true);
            }
        });
    }
